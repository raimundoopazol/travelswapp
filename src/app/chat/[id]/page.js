...
export default function Chat() {
  ...
  return (
    <main style={{ maxWidth: 430, margin: '0 auto', height: '100vh', display: 'flex', flexDirection: 'column', background: '#F4F6F9', fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
      ...
      {/* Rate panel */}
      {showRate && (
        <div style={{ background: '#EFF6FF', padding: '10px 16px', flexShrink: 0, borderBottom: '1px solid #DBEAFE' }}>
          {listing ? (
            <>
              <div style={{ fontSize: 12, color: '#7A8599', marginBottom: 4 }}>Tasa del listing</div>
              <div style={{ fontWeight: 900, fontSize: 16, color: '#0D1B2A' }}>
              ...
              <button onClick={() => sendMessage(`💱 Mi tasa: 1 ${listing.currency_have} = ${formatAmount(listing.rate, listing.currency_want)} ${listing.currency_want}`)}
                style={{ marginTop: 8, background: '#2563EB', border: 'none', borderRadius: 8, padding: '5px 12px', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                Enviar tasa al chat
              </button>
            </>
          ) : (
          ...
          messages.map(m => {
            ...
            return (
              <div key={m.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  maxWidth: '75%', background: isMe ? '#FFD000' : '#fff',
                  borderRadius: isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                  padding: '10px 14px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', fontSize: 14, color: '#0D1B2A'
                }}>
                  {m.message_text}
                  <div style={{ fontSize: 10, color: isMe ? '#7A6500' : '#7A8599', marginTop: 3, textAlign: 'right' }}>
